...
7. NEVER use ** bold anywhere in bullet text — plain prose only`;
    ...
    const handleCopy = async () => {
        if (!itinerary) return;
        try {
            // Build a clean plain-text version of the itinerary
            const cleanText = [
                `ROAMR — ${itinDest} Travel Itinerary`,
                `${'─'.repeat(40)}`,
                `Duration: ${effectiveDays} days  |  Budget: ${effectiveBudget}`,
                useCustomDates && startDate && endDate ? `Dates: ${formatDateRange()}` : '',
                '',
                itinerary
                    .replace(/\*\*/g, '')          // remove bold markers
                    .replace(/^###\s+/gm, '📍 ')  // ### → pin emoji
                    .replace(/^##\s+/gm, '\n──── ') // ## → divider
                    .replace(/^[-*•]\s+/gm, '  · ') // bullets → dots
                    .trim(),
                '',
                `${'─'.repeat(40)}`,
                `Generated by Roamr · roamr.vercel.app`,
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '.75rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                                ...
                                <button className="btn btn-sm" onClick={handleCopy}
                                    style={{
                                        background: copied ? 'var(--green-l)' : 'transparent',
                                        border: `1.5px solid ${copied ? 'rgba(21,128,61,.3)' : 'var(--sand-3)'}`,
